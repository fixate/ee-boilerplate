...
if ( ! defined('ENV'))
{
	require './assets/_config/config.env.php';
}
...
if (isset($config))
{
	...
	$env_config['third_party_path']    = $base_path . $template_path . '/_addons/';
	// $env_config['path_third_themes']   = $base_path . $template_path . '/_addon_themes';
	// $env_config['url_third_themes']    = $template_path . '/_addon_themes';

	/**
	 * Stash directories
	 */
	$env_config['stash_file_basepath'] = $base_path . $template_path . '/_stash/';
	$env_config['stash_file_sync'] = (ENV == 'prod') ? FALSE : TRUE;
	...
	$env_config['stash_default_scope'] = 'user'; // default variable scope if not specified
	...
	$env_config['tmpl_file_basepath']        	= $base_path . $template_path;
	 ...
	 */
	$env_config['webmaster_email']			= 'no-reply@' . $_SERVER['HTTP_HOST'];
